...
<!-- You can begin by using the code in the grey "chunks" within the top-left window of RStudio, then when you have finished you can use it to create an HTML report -->

Within each `{r}` chunk, the R code will run when you press the '▶' button. This is the same R code you can use in any R session or script. Try executing this chunk by clicking the *Run* ('▶') button within the chunk or by placing your cursor inside it and pressing *Ctrl+Shift+Enter*. 

Load the required package `tidyverse` to get started:

```{r setup}
library(tidyverse)
# if `tidyverse` isn't installed, run `install.packages("tidyverse")` first then re-run the `library(tidyverse)` command first.

knitr::opts_chunk$set(messages = FALSE)
```

Once you have run all chunks, when you save the notebook, an HTML file containing the code and output will be saved alongside it. You can click **Preview** above to see the output as you edit and re-run (this will only show you the results of code chunks you have already run), or **Knit to HTML** from the drop-down menu next to Preview to run all chunks again and create an HTML output file.
...
<!-- Within each R chunk, replace `___` with R code to produce the desired outputs. R will alert you with any errors stopping the code from running -->

```{r load_england_data}
england <- read_csv("data/England.csv") # Replace `___` with the path to `England.csv` (hint: in `data` folder)
...
eng_plot <- england %>% 
  ...
  geom_point() + # add scatter points; Hint: You'll find a function for this in the ggplot2 cheatsheet!
...
<!-- (hover over the equation in RStudio to see formatted output, or check HTML 'Preview') -->
...
To do this, we first need to create dummy variables for Time, Level and Trend:
 ...
 - 'Trend' should be 0 before the strategy launch, begin with 1 in 1999 at strategy launch and increment by 1 each year
...
england_its_data <- england %>% 
  mutate(
    Time = Year - 1991,  # To set 1992 to 1, 1993 to 2 etc.
    Level = ifelse(Year >= 1999, 1, 0),
    Trend = ifelse(Year >= 1999, Year - 1998, 0)  # 0 before strategy, 1 in 1999, 2 in 2000 etc.
  )
...
We can now fit a straightforward linear model to our data using the `lm` function and the formula we derived above. This will attempt to test whether the coefficients represent significant level and trend changes at 1999.
...
> In England in 1991 there were approximately **40.8** pregnancies per 1,000 women aged under-18. This **increased** yearly by **0.75 more** pregnancies per year from 1992 to 1998.

(note that in making `Time` begin with 1 in 1992 we implicitly made 1991 our `Time = 0` baseline, rather than using `Year` directly and making 1 B.C.E. our baseline!)

> In 1999 at strategy launch England saw a change in yearly pregnancy rates of **4.02 more** pregnancies per 1,000 women aged under-18. This was a **non-significant** change in level (indepenent of trend changes).

> From 1999 onwards, England saw **a decreasing** trend of **2.30 fewer** pregnancies per 1,000 women aged under-18 per year. This was a **significant** change in trend.

(note that this is relative to the baseline trend)

## Graphing our model

We have fitted straight lines to our data. Now let's plot those lines over our scatter point graph we made above to see how it fits.
...
eng_predict_plot <- eng_plot + 
  geom_line(data = eng_predict, aes(y = fitted, group = Level))  # `group = Level` splits (segments)
...
england_counterfactual <- england_its_data %>% 
  ...
  mutate(Country = "No strategy",
         Level = 0,      # If England had not had a strategy, 
         Trend = 0) %>%  # Level and Trend would have stayed at 0
...
A key advantage of using a control is to adjust for other events and simultaneous interventions which might produce changes which *look* like they're caused by the intervention, but would have happened anyway. In the case of teenage pregnancy in England, there could be quite a few other things affecting rates across the observation period. We could try adjusting for these using Scotland's rates as a control, as Scotland was not exposed to the same strategy as England.

Let's import a dataset of Scotland's rates (from [ISD Scotland](https://www.isdscotland.org)) and compare:

```{r scotland_import}

scotland <- read_csv("data/Scotland.csv") # Read in 'Scotland.csv' (in `data` folder)
...
$Rate = \beta_0+\beta_1*Time+\beta_2*Group+\beta_3*Group*Time+\beta_4*Level+\beta_5*Trend+\beta_6*Group*Level+\beta_7*Group*Trend+\epsilon$

To do this we'll create the dummy variables in our joined dataset:

 - 'Group' will be 1 for England and 0 for Scotland
 - 'Group_time' will be the multiplication of Group and Time - England will have an additional trend on top of the common 'Time' trend controlled for with Scotland
 - 'Group_level' will be the multiplication of Group and Level - again giving England an additional level change only associated with exposure to the strategy.
 - 'Group_trend' as the multiplication of Group and Trend - an additional trend change associated with exposure
...
eng_scot_its <- eng_scot_rates %>%
  mutate(
    ...
    Group = ifelse(Country ==  "England", 1, 0),  # Set to 1 only for England
...
<sup>Side note - `lm` _can_ do interactions for us: `Group*Time` would automatically add the terms `Group + Time + Group:Time`, giving the interactions we need. However it's perhaps simpler to explicitly create the variables and pass them to the function ourselves, both to keep them in the order we specified them and to aid reading the output</sup>

Now see what we can tell from our data.

> In 1991 Scotland had an estimated **41.0** pregnancies per 1,000 women aged under 18, and England had **0.22 fewer**.

> When the strategy launched in England in 1999, Scotland saw **an increase** in pregnancy rates of **3.58** per 1,000 women, whist England's level change was **0.44 more** pregnancies than Scotland. This difference was **not significant**.

> Whilst not exposed to the strategy, Scotland saw a **significant decrease** in trend in pregnancies of **1.84** per 1,000 women each year. England's rates **decreased** by **0.46** *in addition to Scotland's trend change*. This difference was **not significant**.
...
eng_scot_predict <- eng_scot_its %>%
  mutate(fitted = predict(eng_scot_mod)) # pass the new model output to the `predict` function
...
eng_scot_counterfactual <- eng_scot_its %>%
  ...
  mutate(Country = "No strategy",
         Group_level = 0,
         Group_trend = 0) %>%  # In the absence of the strategy both of these terms would be ...?
  mutate(fitted = predict(eng_scot_mod, newdata = .))  # pass in new model
...
You've hopefully now seen what can be tested using an interrupted time series analysis, and the potential advantages of using a control. This is an introduction to using ITS to evaluate a policy, but there will likely be more things to address in analyses like this one:
 ...
 - How well fitted are the linear models, and do other elements (like seasonality and known events) need to be added to the model to give a more accurate picture?
  - Was the model of level and trend change the best one to use? In our example, would omitting the 'Level' change variable (i.e. assuming no 'immediate' effects of the strategy on 1999 rates) better fit the data?
